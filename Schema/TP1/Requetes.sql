...
-- 1) Jointure : lister les joueurs et les jeux qu'ils ont achetés (date + prix)
SELECT j.Pseudo, je.Titre, a.DateAchat, a.Prix
FROM Jouer j
JOIN Achat a ON j.ID_Jouer = a.ID_Jouer
JOIN Jeu je ON a.ID_Jeu = je.ID_Jeu;

-- 2) Agrégation : calculer le prix moyen des achats par joueur
SELECT a.ID_Jouer, AVG(a.Prix) AS Prix_moy
FROM Achat a
GROUP BY a.ID_Jouer;

-- 3) Somme du temps de jeu par joueur (somme des INTERVALs)
SELECT s.ID_Jouer, SUM(s.DureeSession) AS TotalDuree
FROM SessionJeu s
GROUP BY s.ID_Jouer;

-- 4) Top jeux par nombre d'achats
